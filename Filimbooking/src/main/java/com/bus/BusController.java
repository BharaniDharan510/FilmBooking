...
public class BusController {
	...
	// Admin method to add a movie
		@PostMapping("/admin/movies/add")
		public String addMovie(@ModelAttribute("movie") MovieDetails movie, HttpSession session) {
			Customer admin = (Customer) session.getAttribute("user");
			if (admin != null && admin.getBid() == 1) {
		...
		//Admin method to update a movie
		@PostMapping("/admin/movies/update/{id}")
		public String updateMovie(@PathVariable("id") long id, @ModelAttribute("movie") MovieDetails movie, HttpSession session) {
			Customer admin = (Customer) session.getAttribute("user");
			if (admin != null && admin.getBid() == 1) {
		...
		// Admin method to delete a movie
		...
		// Admin method to view all movies
		@GetMapping("/admin/movies")
		public String viewAllMovies(Model m, HttpSession session) {
			Customer admin = (Customer) session.getAttribute("user");
			if (admin != null && admin.getBid() == 1) {
	...
	public String bookSeat(@ModelAttribute("Seat") Seat seat, @RequestParam("movieName") String movieName,
		...
		if (object == null) {
			...
			if (((date.isAfter(currentDate)) || (date.equals(currentDate)))
				...
				dao.sendBookingConfirmation(object.getEmail(), movieName, seatNumbers, date.toString(), time, sum);
				return "redirect:/home";
